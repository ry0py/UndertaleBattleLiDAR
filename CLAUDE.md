...
MCP ツールがセッションに出ていない場合は、Unity Editor が起動していること・
ポート 8080 が LISTENING であることを確認し、**Claude Code を再起動**して新セッションで
MCP ツールを読み込む（MCP ツールはセッション開始時にロードされ、起動済みセッションには
後から反映されない）。

**MCP を使えない場合の代替**: シーン/UI/Prefab 一式は Editor メニュー
`Tools/UndertaleLiDAR/Build Battle Scene`（`Assets/Editor/BattleSceneBuilder.cs`）で
1 クリック生成できる。手書き YAML を使わず正規 API で構築するため再現可能。
...
- シーン/UI/Prefab は `Tools/UndertaleLiDAR/Build Battle Scene` で生成する（実行は要 Unity 操作）。
