...
**MCP 未接続時は作業しない**: ツールが出ていない状態でシーン/Prefab/UI を手で組んだり、
代替の Editor 拡張を作って回避しない。まず接続を回復する。

**ツールが出ていない時の復旧:**
1. Unity Editor が起動済みか確認（MCP For Unity ウィンドウが「Session Active」）。
2. `netstat` で 8080 が LISTENING か確認（サーバー稼働中か）。
3. **新セッションを開始**: VSCode 拡張は `Ctrl+Shift+P`→「Developer: Reload Window」、
   または＋で新規チャット（MCP ツールはセッション開始時のみロード。`/mcp` の reconnect は
   既にリストに出ている時だけ有効で、未ロードのセッションには出てこない）。
4. `/mcp` に `UnityMCP` が Connected で出ることを確認 → `/unity-check` で疎通とコンパイル状態を確認。

注: `UnityMCP` は user スコープ（`~/.claude.json` top-level `mcpServers`）に登録済み。
プロジェクトキーの大文字小文字差（`c:`/`C:`）で読み込まれない問題を避けるため、
プロジェクトスコープではなく user スコープを正とする。
